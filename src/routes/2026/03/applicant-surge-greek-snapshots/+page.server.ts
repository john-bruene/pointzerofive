import rawStoryDataJson from '$data/us-top200-longitudinal.json';
...
const rawStoryData = rawStoryDataJson as RawStoryData;
...
function parseGreekPercent(rawValue: string | null, yesNoFlag: string | null): number | null {
	...
	if (cleaned === '' || cleaned === 'Not Reported' || cleaned === 'N/A') {
		return null;
	}
	if (cleaned === '-' && yesNoFlag === 'N') {
		return 0;
	}
...
function buildYearlySeries(storyData: RawStoryData): { yearly: YearPoint[]; balancedUnitIds: Set<number> } {
	const admissions = storyData.admissions_panel;
	const top50UnitIds = new Set(
		storyData.top200.filter((row) => row.rank_2022_by_applicants <= 50).map((row) => row.unitid)
	);
...
function buildGreekSeries(storyData: RawStoryData): {
	greekSnapshots: GreekPoint[];
	coverageChecks: CoverageCheckPoint[];
} {
	...
	for (const snapshotDate of [...groupedByDate.keys()].sort((a, b) => a.localeCompare(b))) {
		...
		const usableRows = matchedRows
			.map((row) => {
				const fraternity =
					row.pct_fraternity ??
					parseGreekPercent(row.fraternity_raw, row.fraternities_yes_no);
				const sorority =
					row.pct_sorority ??
					parseGreekPercent(row.sorority_raw, row.sororities_yes_no);
				return {
					rank: row.rank_2022_by_applicants,
					fraternity,
					sorority
				};
			})
...
function buildStoryPayload(storyData: RawStoryData): StoryPayload {
	...
	const admissionsRowsExpected =
		...
		storyData.summary.top200_count;
	...
	return {
		...
		highlights: {
			...
			admitRatePandemicJump:
				yearly.some((point) => point.year === 2019) && yearly.some((point) => point.year === 2020)
					? (yearly.find((point) => point.year === 2020)?.admitRate ?? 0) -
						(yearly.find((point) => point.year === 2019)?.admitRate ?? 0)
					: null,
		...
		checks: {
			...
			coverageChecks,
			coverageChecksAgree: coverageChecks.every((point) => point.matchesAgree),
...
export const load: PageServerLoad = () => {
	const story = buildStoryPayload(rawStoryData);
	return { story };
};
